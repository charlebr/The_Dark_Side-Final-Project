make install -> Compiles the source code
make test -> runs Google Test
make run -> runs the actual program
...
thrust::device_vector<float> d_vec((int)size/step);

thrust::device_vector<int> h_vec((int)size/step);

thrust::sequence(h_vec.begin(),h_vec.end(),0);

fill f(step);

thrust::transform(h_vec.begin(),h_vec.end(),d_vec.begin(),f());
